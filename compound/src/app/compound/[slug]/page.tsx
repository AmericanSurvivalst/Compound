...
import { ExternalLink, AlertTriangle, CheckCircle, Lock, ArrowLeft } from 'lucide-react';
...
export default function CompoundPage() {
      ...
      <div className="min-h-screen bg-[var(--bg-primary)]">
        ...
        <div className="flex items-center justify-center py-32">
          <div className="text-center">
            ...
            <Link href="/compounds" className="text-[var(--accent-primary)] hover:underline">
  ...
  // Related compounds (same category)
  const relatedCompounds = compounds
    .filter(c => c.category === compound.category && c.id !== compound.id)
    .slice(0, 3);

  // JSON-LD for compound
  const compoundJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: compound.name,
    description: compound.one_liner,
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Breadcrumb */}
        <nav className="mb-6">
          <Link href="/compounds" className="flex items-center gap-2 text-sm text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] transition-colors">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {compound.goals.map(goal => (
            <span key={goal} className="px-3 py-1 rounded-full text-sm bg-[var(--bg-secondary)] text-[var(--text-secondary)]">
        ...
        <section className="mb-12">
          ...
          <div className="p-6 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border)]">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[var(--text-secondary)]">Verified sources available with Pro</span>
            </div>
            <Link 
              href="/auth"
        ...
        {relatedCompounds.length > 0 && (
          <section className="mb-12">
            <h2 className="text-xl font-semibold mb-4">Related Compounds</h2>
            <div className="grid sm:grid-cols-3 gap-4">
              {relatedCompounds.map((related) => (
                <Link
                  key={related.id}
                  href={`/compound/${related.slug}`}
                  className="p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border)] hover:border-[var(--accent-primary)] transition-colors"
                >
                  <h3 className="font-medium mb-1">{related.name}</h3>
                  <p className="text-sm text-[var(--text-tertiary)] line-clamp-2">{related.one_liner}</p>
                </Link>
              ))}
            </div>
          </section>
      ...
      {/* Disclaimer */}
      <div className="border-t border-[var(--border)] bg-red-500/5">
